let Co2okWidget = {
  ...
  widgetGenerator: function (widgetContainer, totalCompensatedData) {
      ...
      if (totalCompensatedData <500) {
        ...
        var compensatietekst = `The webshops connected to CO₂ok have together prevented <strong> ${compensatiewidget .toFixed(1)} </strong>tonnes of CO₂ emissions <br><br>= <strong>${(compensatiewidget * 5000).toFixed(0)} </strong>km flying`;
      }
      else {
        var compensatiewidget  = totalCompensatedData / 1000;
        var compensatietekst = `This webshop has prevented <strong>${compensatiewidget .toFixed(1)} </strong>tonnes CO₂-emissions <br><br>= <strong>${(compensatiewidget * 5000).toFixed(0)} </strong>km vliegen`
      }
